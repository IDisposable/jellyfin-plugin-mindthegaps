...
| **Scan TMDB watchlist** (`ScanTmdbWatchlist`) | Off | Surfaces the unowned movies and shows on the connected TheMovieDb account's watchlist. Needs **your own TMDB API key** and a connected account (see Data sources). No vote floor applies: you put these there deliberately. | The watchlist is ignored. |
...
| **Scan Trakt watchlist** (`ScanTraktWatchlist`) | Off | Surfaces the unowned movies and shows on a Trakt user's watchlist. Needs the **Trakt Client ID** under Data sources and the username below; Trakt serves a public profile's watchlist without OAuth. | The watchlist is ignored. |
...
| **Scan Discogs wantlist** (`ScanDiscogsWantlist`) | Off | Surfaces the unowned releases on a Discogs wantlist as Music gaps. Needs the **Discogs token** and the username below. | The wantlist is ignored. |
...
| **Scan OpenLibrary want to read** (`ScanOpenLibraryWantToRead`) | Off | Surfaces the unowned works on an OpenLibrary "Want to Read" shelf as Books gaps. Needs no key. | The shelf is ignored. |
...
| **Scan TheTVDB favorites** (`ScanTvdbFavorites`) | Off | Surfaces the unowned series favorited on TheTVDB. Needs the **TheTVDB API key** and the **subscriber PIN** under Data sources. Expect few results: a favorite is usually a show you already hold. | The favorites are ignored. |
| **Scan IMDb lists** (`ScanImdbLists`) | Off | Surfaces the unowned titles (movies and shows) from the IMDb watchlists and lists named below. Needs no key: IMDb's own API serves any list its owner has published. | The IMDb ids are ignored. |
