...
public sealed class RelatedMissingService
{
    ...
    /// <summary>
    /// Rehydrates one of the item's related gaps by id, for a detail view or a Send. Recomputed server-side
    /// from the same inputs the page listed.
    /// </summary>
    /// <param name="itemId">The Jellyfin item id.</param>
    ...
    {
        var item = _libraryManager.GetItemById<BaseItem>(itemId);
        if (item is not (Movie or Series) || string.IsNullOrEmpty(gapId))
        {
            return null;
        }
    ...
    {
        if (!item.TryGetProviderId(ProviderIds.Tmdb, out var raw)
