...
internal static class MissingTitleBuilder
{
    ...
    public static MissingTitle? ToTitle(GapItem gap, string? role, string? because)
    {
        ...
        if (TmdbId(gap) is not int tmdbId)
        {
            return null;
        }
    ...
    /// <summary>
    /// Reads the gap's TMDB id.
    /// </summary>
    /// <param name="gap">The gap.</param>
    /// <returns>The id, or <see langword="null"/>.</returns>
    public static int? TmdbId(GapItem gap)
    {
        ArgumentNullException.ThrowIfNull(gap);
        ...
        foreach (var pair in gap.ProviderIds)
        {
            if (string.Equals(pair.Key, ProviderIds.Tmdb, StringComparison.OrdinalIgnoreCase)
                && int.TryParse(pair.Value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var id)
                && id > 0)
            {
                return id;
            }
    ...
    public static string? Because(GapItem gap)
    {
        ArgumentNullException.ThrowIfNull(gap);
        ...
        {
            if (!string.IsNullOrWhiteSpace(other.Name) && !names.Contains(other.Name.Trim(), StringComparer.OrdinalIgnoreCase))
            {
                names.Add(other.Name.Trim());
            }
        }
    ...
    public static IReadOnlyList<MissingTitle> NewestFirst(IEnumerable<MissingTitle> titles)
    {
