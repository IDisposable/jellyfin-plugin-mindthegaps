            ...
            (function () {
                ...
                var DIAG_MATCH_EXPLAINER = [
                    ...
                    'For an episode or season, ownership is not id-matched. For an episode the diagnosis first checks whether the season and episode number is among the ones the library owns for the series: if it is, the episode is not actually missing (a stale gap, or a numbering the cross-check disagrees on). Otherwise it compares the missing air year against the earliest and latest years of the episodes already owned, so a late season reads as missing while a reboot decades apart reads as a different, same-named series the owning item is mis-tagged as.'
                ];
